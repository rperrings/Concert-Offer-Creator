(() => {
    ...
    offerTypeSelect.addEventListener('change', () => {
        ...
        if (type === 'Festival') {
            ...
            eventDetailsField.style.display = 'block';
            mealAlcoholFields.style.display = 'none';
        } else if (type === 'Hard Tickets Theater') {
    ...
    function updateEarningsPotential() {
        ...
        const dealStructure = document.getElementById('theaterDealStructure').value;
    ...
    document.getElementById('theaterHeadlinerFee').addEventListener('input', updateTotalTalentCost);
    ...
    // --- Deal Structure dropdown toggle ---
    const dealStructureSelect = document.getElementById('theaterDealStructure');
    ...
    dealStructureSelect.addEventListener('change', () => {
        if (dealStructureSelect.value) {
            dealStructureValueInput.style.display = 'block';
            dealStructureValueInput.focus();
            const placeholders = {
                'Flat Guarantee': 'Guarantee amount already set above',
                'Versus': 'e.g., 85%',
                'Plus Deal': 'e.g., 10%',
                'Straight Percentage': 'e.g., 100%',
            };
            dealStructureValueInput.placeholder = placeholders[dealStructureSelect.value] || 'Enter value';
        } else {
            dealStructureValueInput.style.display = 'none';
            dealStructureValueInput.value = '';
        }
        updateEarningsPotential();
    });
    ...
    const accommodationsIncludedSelect = document.getElementById('accommodationsIncluded');
    const accommodationsFieldsDiv = document.getElementById('accommodationsFields');

    accommodationsIncludedSelect.addEventListener('change', () => {
        if (accommodationsIncludedSelect.value === 'Yes') {
    ...
    const travelOtherInput = document.getElementById('travelOther');

    travelSelect.addEventListener('change', () => {
        if (travelSelect.value === 'other') {
            travelOtherInput.style.display = 'block';
            travelOtherInput.focus();
        } else {
            travelOtherInput.style.display = 'none';
            travelOtherInput.value = '';
        }
    });

    // --- Transportation "Other" toggle + Ground Type cascade ---

    const transportationSelect = document.getElementById('transportation');
    const transportationOtherInput = document.getElementById('transportationOther');
    const groundTypeField = document.getElementById('groundTypeField');
    const groundTypeSelect = document.getElementById('groundType');
    const uberBuyoutField = document.getElementById('uberBuyoutField');
    const uberBuyoutAmountSelect = document.getElementById('uberBuyoutAmount');

    transportationSelect.addEventListener('change', () => {
        if (transportationSelect.value === 'other') {
            transportationOtherInput.style.display = 'block';
            transportationOtherInput.focus();
            groundTypeField.style.display = 'none';
            uberBuyoutField.style.display = 'none';
        } else if (transportationSelect.value === 'Ground provided') {
            transportationOtherInput.style.display = 'none';
            transportationOtherInput.value = '';
            groundTypeField.style.display = 'block';
            // Show/hide Uber buyout based on current ground type selection
            uberBuyoutField.style.display = groundTypeSelect.value === 'Uber Buyout' ? 'block' : 'none';
        } else {
            transportationOtherInput.style.display = 'none';
            transportationOtherInput.value = '';
            groundTypeField.style.display = 'none';
    ...
    groundTypeSelect.addEventListener('change', () => {
        if (groundTypeSelect.value === 'Uber Buyout') {
    ...
    form.addEventListener('submit', (e) => {
        ...
        const accommodationsIncluded = accommodationsIncludedSelect.value;
        ...
        const nonCompeteDays = document.getElementById('nonCompeteDays').value;
        ...
        const travel = travelSelect.value === 'other'
            ? travelOtherInput.value.trim()
            : travelSelect.value;
        let transportation;
        if (transportationSelect.value === 'other') {
            transportation = transportationOtherInput.value.trim();
        } else if (transportationSelect.value === 'Ground provided') {
            if (groundTypeSelect.value === 'Uber Buyout') {
                const uberAmount = parseInt(uberBuyoutAmountSelect.value);
                transportation = `Uber Buyout — ${formatCurrency(uberAmount)}`;
            } else {
                transportation = groundTypeSelect.value;
            }
        } else {
            transportation = transportationSelect.value;
        }
        ...
        if (offerType === 'Festival') {
            ...
            dealStructure = document.getElementById('dealStructure').value || '';
            ...
            eventDetails = document.getElementById('eventDetails').value.trim();
        } else if (offerType === 'Hard Tickets Theater') {
            // Theater has its own data gathering — handle separately
            const onSaleTimeHour = document.getElementById('theaterOnSaleTimeHour').value;
            const onSaleTime = onSaleTimeHour
                ? `${onSaleTimeHour}:${document.getElementById('theaterOnSaleTimeMinute').value} ${document.getElementById('theaterOnSaleTimePeriod').value}`
                : '';
            ...
            const theaterData = {
                ...
                agentAddress: document.getElementById('theaterAgentAddress').value.trim(),
                agentPhone: document.getElementById('theaterAgentPhone').value.trim(),
                ...
                doorsTime: `${document.getElementById('theaterDoorsHour').value}:${document.getElementById('theaterDoorsMinute').value} ${document.getElementById('theaterDoorsPeriod').value}`,
                showTime: `${document.getElementById('theaterShowHour').value}:${document.getElementById('theaterShowMinute').value} ${document.getElementById('theaterShowPeriod').value}`,
                curfewTime: `${document.getElementById('theaterCurfewHour').value}:${document.getElementById('theaterCurfewMinute').value} ${document.getElementById('theaterCurfewPeriod').value}`,
                ...
                ageRestriction: document.getElementById('theaterAgeRestriction').value,
                // Offer Details
                ...
                dealStructure: document.getElementById('theaterDealStructure').value,
    ...
    function generatePDF(data) {
        ...
        // --- Hard Tickets Theater: Live Nation SHOW OFFER layout (bordered) ---
        if (data.offerType === 'Hard Tickets Theater') {
            const tMargin = 0.5;
            ...
            if (data.agentAddress) { doc.text(data.agentAddress, rightX, rightY, { align: 'right' }); rightY += 0.14; }
            if (data.agentPhone) { doc.text(data.agentPhone, rightX, rightY, { align: 'right' }); rightY += 0.14; }
        ...
        // Non-Compete (paragraph) — not applicable for Private/Public Corporate Events
        if (data.offerType !== 'Private Event' && data.offerType !== 'Public Corporate Event') {
            const milesWord = numberToWords(parseInt(data.nonCompeteMiles));
            const daysWord = numberToWords(parseInt(data.nonCompeteDays));
            const nonCompete = `Artist shall not host or perform at any other nightclub, bar, private party, or lounge within ${milesWord} (${data.nonCompeteMiles}) miles of ${data.venueCity} during the Engagement Period or within ${daysWord} (${data.nonCompeteDays}) days before or after such period.`;
