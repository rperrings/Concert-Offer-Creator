(() => {
    ...
    const marketingDeliverables = document.getElementById('marketingDeliverables');
    ...
    offerTypeSelect.querySelectorAll('button').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            marketingDeliverables.style.display = 'none';
            ...
            if (type === 'Festival') {
                ...
                marketingDeliverables.style.display = 'block';
    ...
    // Default to Hard Tickets on page load
    const defaultBtn = offerTypeSelect.querySelector('[data-value="Hard Tickets"]');
    if (defaultBtn) defaultBtn.click();

    // --- Hospitality Rider auto-set ---

    function applyHospitalityRider() {
        if (getActiveValue(hospitalityRiderSelect) === 'Yes') {
    ...
    hospitalityRiderSelect.querySelectorAll('button').forEach(btn => {
        btn.addEventListener('click', () => {
            hospitalityRiderSelect.querySelectorAll('button').forEach(b => b.classList.remove('active'));
            btn.classList.add('active');
            applyHospitalityRider();
        });
    });

    // --- Marketing Deliverables toggle ---
    const marketingDeliverablesToggle = document.getElementById('marketingDeliverablesToggle');
    const marketingDeliverablesFieldsDiv = document.getElementById('marketingDeliverablesFields');
    const deliverableSliders = document.getElementById('deliverableSliders');

    marketingDeliverablesToggle.querySelectorAll('button').forEach(btn => {
        btn.addEventListener('click', () => {
            marketingDeliverablesToggle.querySelectorAll('button').forEach(b => b.classList.remove('active'));
            btn.classList.add('active');
            marketingDeliverablesFieldsDiv.style.display = btn.dataset.value === 'Yes' ? 'block' : 'none';
        });
    });

    // --- Marketing Deliverables pill selection + quantity sliders (shared, excludes theater) ---
    const deliverableOtherText = document.getElementById('deliverableOtherText');
    document.querySelectorAll('.deliverables-pills .pill:not(.theater-pill)').forEach(pill => {
        pill.addEventListener('click', () => {
            pill.classList.toggle('selected');
            const name = pill.dataset.deliverable;
            if (name === 'Other') {
                deliverableOtherText.style.display = pill.classList.contains('selected') ? 'block' : 'none';
                if (!pill.classList.contains('selected')) deliverableOtherText.value = '';
                return;
            }
            if (pill.classList.contains('selected')) {
                const row = document.createElement('div');
                row.className = 'deliverable-slider-row';
                row.dataset.deliverable = name;
                ...
                slider.addEventListener('input', () => {
                    qtySpan.textContent = slider.value;
                });
                row.appendChild(label);
                row.appendChild(slider);
                deliverableSliders.appendChild(row);
            } else {
                const existing = deliverableSliders.querySelector(`[data-deliverable="${name}"]`);
                if (existing) existing.remove();
            }
        });
    });

    // --- Theater Marketing Deliverables toggle + pills ---
    const theaterMarketingToggle = document.getElementById('theaterMarketingToggle');
    const theaterMarketingFieldsDiv = document.getElementById('theaterMarketingFields');
    const theaterDeliverableSliders = document.getElementById('theaterDeliverableSliders');

    theaterMarketingToggle.querySelectorAll('button').forEach(btn => {
        btn.addEventListener('click', () => {
            theaterMarketingToggle.querySelectorAll('button').forEach(b => b.classList.remove('active'));
            btn.classList.add('active');
            theaterMarketingFieldsDiv.style.display = btn.dataset.value === 'Yes' ? 'block' : 'none';
        });
    });

    const theaterDeliverableOtherText = document.getElementById('theaterDeliverableOtherText');
    document.querySelectorAll('.theater-pill').forEach(pill => {
        pill.addEventListener('click', () => {
            pill.classList.toggle('selected');
            const name = pill.dataset.deliverable;
            if (name === 'Other') {
                theaterDeliverableOtherText.style.display = pill.classList.contains('selected') ? 'block' : 'none';
                if (!pill.classList.contains('selected')) theaterDeliverableOtherText.value = '';
                return;
            }
            if (pill.classList.contains('selected')) {
                const row = document.createElement('div');
                ...
                row.appendChild(label);
                row.appendChild(slider);
                theaterDeliverableSliders.appendChild(row);
            } else {
                const existing = theaterDeliverableSliders.querySelector(`[data-deliverable="${name}"]`);
                if (existing) existing.remove();
            }
        });
    });
    ...
    function getSelectedDeliverables(container) {
        const parent = container || document.getElementById('deliverableSliders');
        const rows = parent.querySelectorAll('.deliverable-slider-row');
        const items = Array.from(rows).map(row => ({
            name: row.dataset.deliverable,
            quantity: parseInt(row.querySelector('input').value),
        }));
        // Check for "Other" text input
        const otherInput = parent === theaterDeliverableSliders
            ? theaterDeliverableOtherText
            : deliverableOtherText;
        if (otherInput && otherInput.style.display !== 'none' && otherInput.value.trim()) {
            items.push({ name: otherInput.value.trim(), quantity: null });
        }
        return items;
    }
    ...
    form.addEventListener('submit', (e) => {
        ...
        if (offerType === 'Festival') {
            ...
            hospitalityRider = getActiveValue(hospitalityRiderSelect);
            ...
            const theaterData = {
                ...
                // Marketing Deliverables (prepend to Offer Notes)
                theaterMarketingIncluded: getActiveValue(theaterMarketingToggle),
                theaterMarketingItems: getSelectedDeliverables(theaterDeliverableSliders),
                // Notes
        ...
        generatePDF({
            ...
            guestCount,
            eventDetails,
            marketingDeliverablesIncluded: getActiveValue(marketingDeliverablesToggle),
            marketingDeliverablesItems: getSelectedDeliverables(),
        });
    });
    ...
    function generatePDF(data) {
        ...
        if (data.offerType === 'Hard Tickets') {
            ...
            // ============ 10. OFFER NOTES ============
            // Prepend marketing deliverables to offer notes if selected
            let offerNotesText = data.offerNotes || '';
            if (data.theaterMarketingIncluded === 'Yes' && data.theaterMarketingItems && data.theaterMarketingItems.length > 0) {
                const marketingLine = 'Marketing Deliverables: ' + data.theaterMarketingItems
                    .map(item => item.quantity ? `${item.quantity}x ${item.name}` : item.name)
                    .join(', ');
                offerNotesText = offerNotesText ? marketingLine + '\n\n' + offerNotesText : marketingLine;
            }
            ...
            if (offerNotesText) {
                ...
                doc.setFontSize(8);
                doc.setTextColor(30, 30, 30);
                const noteLines = doc.splitTextToSize(offerNotesText, tWidth - cellPad * 2);
        ...
        // Marketing Deliverables (not for Private Event)
        if (data.marketingDeliverablesIncluded === 'Yes' && data.marketingDeliverablesItems && data.marketingDeliverablesItems.length > 0) {
            const itemsText = data.marketingDeliverablesItems
                .map(item => item.quantity ? `${item.quantity}x ${item.name}` : item.name)
                .join(', ');
            y = drawField(doc, 'Marketing Deliverables:', itemsText, labelX, valueX, y, maxValueWidth);
        }
